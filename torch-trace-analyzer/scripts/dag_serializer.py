...
def _serialize_io_node(node: DagNode, io_subtype: str) -> dict:
    if io_subtype == "input":
        if not isinstance(node, InputNode) or not isinstance(node.attr, ForwardArgAttr):
            raise RuntimeError(f"node {node.node_id} is not a ForwardArg input node")
        ...
        if not isinstance(node, ResultNode) or not isinstance(node.attr, ResultAttr):
            raise RuntimeError(f"node {node.node_id} is not a Result output node")
...
def _serialize_module_node(node: ModuleNode, dag: DAG, registry: dict[int, DagNode]) -> dict:
    ...
    return {
        ...
        "call_loc": _serialize_call_loc(node.call_loc),
        "attr_type": attr_type,
        "inner_dag": serialize_dag(node.inner_dag, registry) if node.inner_dag is not None else None,
    }
