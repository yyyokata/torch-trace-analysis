...
两份规划层次不同，不是互斥关系：
- **旧 Phase C/D/F/G** = 架构清理/技术债收口（清除 `_parse_local_stmt`、regex fallback、加 memo、统一失败信号）
- **新 Step2-P1/P2/缺口C/D** = ConstantResolver 能力补齐（直接影响 DAG 节点展开正确性）

| 旧 Phase | 对应新规划 | 说明 |
|---|---|---|
| Phase C（删 `_parse_local_stmt`，搬 8 个 helper） | 无直接对应 | 纯架构清理，不影响 correctness；是 P1/P2 的工程地基，但不是前置阻塞 |
| Phase D（删所有 regex fallback） | Step2-P1 + Step2-P2 的上层 | P1/P2 是 Phase D 里最影响正确性的两个子集，Phase D 剩余部分可推迟 |
| Phase F（EvalTrace + `--debug-eval`） | 缺口C | 高度重叠；缺口C 是 Phase F 的当前落地版，建议先做 reason counter，后做 CLI |
| Phase G（`_eval_cache` memo） | 已完成 | 脚本里 `_eval_cache` 已实现，key 设计比文档更完整，不需要排进 Step2 |

缺口D（清理 `class_init_param_anno`）：不属于旧 Phase，但和 Phase D 的"失败保守不猜值"原则一致。当前字段虽然没被使用，但注释还描述着被禁止的 fallback 方向，需要提前清掉，防止后续 P1/P2 改 resolver 时误用。
...
| P1 | 实例级普通形参传播（`Block(2)→range(n)=2`） | Step2-P1 | 最明确的 correctness 缺口，`_pass4_callsite_bind` 已有结构基础 |
| P2 | list_len 多级属性链（`self.config.names`） | Step2-P2 | 当前脚本注释明确写"not supported in PR1" |
| P3 | failure reason counter（resolver 内部加 `_fail(reason)`） | 缺口C / Phase F 轻量版 | 和 P1/P2 并行，先不做 `--debug-eval` CLI |
...
| P5 | 全量 regex fallback 删除（`re.search`、snippet parse 残留） | Phase D 剩余 |
